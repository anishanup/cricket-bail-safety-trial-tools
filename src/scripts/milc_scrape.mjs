...
// no ball bowled (abandoned before the start, or an empty duplicate record): not a game played
const abandoned = rows.filter((r) => !r.events.length && /^[^:]*:\s*0\/0\(0\)\s*$/.test(r.summary || ""));
...
for (const r of withGuards) for (const e of r.events) { if (!e.local) e.local = e.at ? localHM(e.at, tzOf(r.ground)) : e.time; if (e.link == null) e.link = ""; }
